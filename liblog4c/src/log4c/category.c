								  ...
								  va_list a_args)
{
	...
	*/
	...
	xp_gettimeofday(&evt.evt_timestamp, NULL);
...
LOG4C_API void log4c_category_log(const log4c_category_t* a_category,
								  int a_priority,
								  const char* a_format,
								  ...)
{
...
LOG4C_API void log4c_category_log_name(const char* category_name,
									   ...
									   ...)
{
	const log4c_category_t* a_category = log4c_category_get(category_name);
	if (log4c_category_is_priority_enabled(a_category, a_priority)) {
		va_list va;
		va_start(va, a_format);
		log4c_category_vlog(a_category, a_priority, a_format, va);
									   ...
									   va_list a_args)
{
	const log4c_category_t* a_category = log4c_category_get(category_name);
	if (log4c_category_is_priority_enabled(a_category, a_priority)) {
		log4c_category_vlog(a_category, a_priority, a_format, a_args);
	}
}
