									   ...
									   long current_file_size)
{
   ...
   */
   len = (long) strlen(a_event->evt_rendered_msg);
...
static int sizewin_rollover(log4c_rollingpolicy_t *this, FILE ** current_fpp ){
	...
	*/
	...
	if ( !swup || !swup->sw_logdir){
		...
		sd_debug("current file descriptor '%x'", (*current_fpp));		
	}
	sd_debug("]");
	return(rc);
}
										   ...
										   long i)
{
	...
	sprintf(tmp, "%ld", i);
	filename_len = (long) strlen(swup->sw_logdir) + 1 +
		(long) strlen(swup->sw_files_prefix) + 1 +
		(long) strlen(tmp) + 1 + 10; /* a margin */
...
//	struct stat	info;
	...
	while( i <swup->sw_conf.swc_file_max_num_files &&
		xp_file_exist(swup->sw_filenames[i])  ) {
			i++;
	}

	if ( i == 0 ){
		if ( xp_file_exist(swup->sw_filenames[i]) ) {
			return(0);
		} else {
			return(-1);
		}
...
static int sizewin_open_zero_file(char *filename, FILE **fpp ){
	...
	if ( (*fpp = fopen(filename, "w+")) == NULL){
		...
		sd_error("failed to open zero file '%s'--defaulting to stderr", filename);    
		*fpp = stderr;
		rc = 1;
	}  
