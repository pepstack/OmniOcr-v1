...
/******************************************************************************/
extern int log4c_init(const char * log4crc_path)
{
    size_t i;
    int ret = 0;
    ...
    {
        int i;
        const char* lpszRCPATH  = log4crc_path;
        const char* lpszHOME    = "";

        /* Fix on WinCE by cheungmine */
#if defined(_WIN32_WCE) || defined(WINCE)
        TCHAR WCE_RCPATH[MAX_PATH+1];
        char  ACE_RCPATH[MAX_PATH+1];
#endif

#ifdef LOG4C_RCPATH
        lpszRCPATH = LOG4C_RCPATH;
#endif
        if (! lpszRCPATH) {
            lpszRCPATH  = getenv("LOG4C_RCPATH") ? getenv("LOG4C_RCPATH") : lpszRCPATH;
        }
        lpszHOME    = getenv("HOME") ? getenv("HOME") : "";

        /* Fix on WinCE by cheungmine */
#if defined(_WIN32_WCE) || defined(WINCE)
        if (strlen(lpszRCPATH) == 0) {
            if (SHGetSpecialFolderPath(0, WCE_RCPATH, CSIDL_WINDOWS, FALSE)) {
                WCE_RCPATH[MAX_PATH] = 0;
                if (WideCharToMultiByte(CP_ACP, 0, WCE_RCPATH, -1, ACE_RCPATH, MAX_PATH, 0, 0) > 0) {
                    ACE_RCPATH[MAX_PATH] = 0;
                    lpszRCPATH = ACE_RCPATH;
                }
            }
        }
#endif
        ...
        {
            snprintf(rcfiles[2].name, sizeof(rcfiles[2].name) - 1, "%s/.log4crc",
